...
//베팅하는 함수
int LetsBet(int Money, int CurrentBet)
{
	printf("\n현재 소지금 : [%d]원\n", Money);
	printf("현재 배팅 금액 : [%d]원\n", CurrentBet);
	Money -= CurrentBet;
...
int ChooseOddOrEven(int Select)
{
...
int RandOddOrEven(int Result)
{
	Result = rand() % 2 + 1;
	return Result;
}

int PirzeOrKeepoing(int MinimumBet, int CurrentBet, int Money)
{
	int WinPrice = CurrentBet * 2;	// 승리 상금은 현재 배팅금액의 두배
	...
	int KeepGoing = 0;
		...
		if (KeepGoing == 1)
		{
			CurrentBet = WinPrice;		// 상금을 다시 그대로 배팅
		}
		else
		{
			Money += WinPrice;			// 상금을 받고
			CurrentBet = MinimumBet;	// 100원부터 다시 배팅
		}
	return WinPrice;
}
