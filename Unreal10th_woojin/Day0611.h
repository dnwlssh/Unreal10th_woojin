...
public:
	...
	void PrintVector(std::vector<int>& InData);
	void TestLambda();
};
...
//		- 함수객체(Functor) : 알고리즘의 정렬 순서 같은 것을 커스텀으로 지정할 수 있게됨(일반적으로 람다식으로 만듬)
...
//	- 기본 구조 
//		[캡처](매개변수) -> (반환타입) { 함수 본문 }
//		1. [캡처](Capture Clause) : 람다 함수 외부의 변수에 어떻게 접근 할 지 지정하는 부분
//			-[]			: 외부 변수에 접근하지 않습니다.
...
//		3. - > 반환타입(Return Type) : 람다 함수의 반환 타입을 명시적으로 지정합니다. 대부분의 경우 컴파일러가 return 문을
//		4. { 함수 본문 }(Funtion Body) : 실제 실행될 코드를 작성하는 부분입니다.
//	- 주의 사항
